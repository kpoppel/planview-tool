...
6. Choose **New template** to define a named daily or weekly template. Daily templates have a Monday-Friday hours column for every activity; weekly templates have one weekly total per activity. Save it, then select the template from **Entry type** to fill all configured activities at once. Daily filling matches each rendered Planview date to the corresponding template weekday, even when only one day such as Monday is shown. Weekly totals are prorated by the number of visible working weekdays, so a one-day table receives one fifth of the weekly total. When a template is selected, choose an **Activity to adjust** and enter a positive or negative **Time adjustment** in 0.5-hour increments. Daily adjustments are distributed across visible, non-holiday weekdays; weekly adjustments are added to the selected activity's weekly total. Templates and the default-template setting are stored in the page's browser `localStorage`; choose **Edit selected** to update one.
